...
func GetJual(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		// ( DB.Model : wadah)
		// preload : untuk memanggil struct lainnya  yang ada di dalam sebuah struct
...
func GetRelasi(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		url := r.URL.String()
		...
		var buff []detail
		DB.Model(&models.Kategori{}).Select("barangs.id_barang, barangs.nama_barang, barangs.harga, kategoris.id_kategori, kategoris.nama_kategori, juals.id_jual").Joins("inner join barangs on kategoris.id_kategori = barangs.kategori_id inner join juals on juals.barang_id = barangs.id_barang").Where(&models.Kategori{Id_kategori: put}).Scan(&buff)
